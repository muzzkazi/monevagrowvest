...
const AIRecommendations = ({ goals, riskProfile, onComplete }: AIRecommendationsProps) => {
  ...
  const handleDownloadReport = () => {
    // Direct download without contact form
    toast({
      title: "Report Downloaded",
      description: "Your detailed investment strategy report has been downloaded successfully.",
    });
    
    // Create and download a simple report file
    const reportContent = `
MONEVA INVESTMENT STRATEGY REPORT
=================================

Generated for: Investment Goals
Date: ${new Date().toLocaleDateString()}

INVESTMENT SUMMARY:
- Total Monthly SIP Required: ₹${totalMonthlySIP.toLocaleString()}
- Total Target Amount: ₹${(totalTargetAmount / 100000).toFixed(1)}L
- Investment Horizon: ${Math.max(...goals.map(g => g.timeHorizon))} years
- Risk Profile: ${riskProfile}

GOAL BREAKDOWN:
${goalSIPs.map(goal => `- ${goal.name}: ₹${goal.monthlySIP.toLocaleString()}/month for ${goal.timeHorizon} years (Target: ₹${(goal.targetAmount / 100000).toFixed(1)}L)`).join('\n')}

ASSET ALLOCATION:
${assetAllocation.map(asset => `- ${asset.category}: ${asset.percentage}%`).join('\n')}

For detailed fund recommendations and implementation support, contact us at +91 80878 55185

---
Generated by Moneva AI Planning Tool
Contact: contact@moneva.in | +91 80878 55185
`;
    ...
    link.href = url;
    link.download = `Moneva_Investment_Strategy_${new Date().toISOString().split('T')[0]}.txt`;
