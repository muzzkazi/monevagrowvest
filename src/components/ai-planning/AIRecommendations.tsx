...
const AIRecommendations = ({ goals, riskProfile, onComplete }: AIRecommendationsProps) => {
  ...
  const handleDownloadReport = () => {
    ...
    toast({
      ...
      description: "Your comprehensive investment strategy report is being generated...",
    });
    ...
    try {
      ...
      // 2. GOAL BREAKDOWN
      checkPageBreak(20 + (goalSIPs.length * 8));
      pdf.setFontSize(14);
      pdf.text('2. GOAL BREAKDOWN', 20, yPosition);
      yPosition += 8;
      
      pdf.setFontSize(10);
      goalSIPs.forEach((goal, index) => {
        const goalText = `${index + 1}. ${goal.name}: ₹${goal.monthlySIP.toLocaleString()}/month for ${goal.timeHorizon} years (Target: ₹${(goal.targetAmount / 100000).toFixed(1)}L)`;
        pdf.text(goalText, 25, yPosition);
        yPosition += 6;
      });
      ...
      // 3. ASSET ALLOCATION
      checkPageBreak(20 + (assetAllocation.length * 6));
      pdf.setFontSize(14);
      pdf.text('3. STRATEGIC ASSET ALLOCATION', 20, yPosition);
      yPosition += 8;
      ...
      checkPageBreak(30);
      pdf.setFontSize(14);
      pdf.text('4. RECOMMENDED FUND SELECTION', 20, yPosition);
      yPosition += 8;
      ...
      assetAllocation.forEach(asset => {
        checkPageBreak(20);
        pdf.setFontSize(12);
        pdf.text(`${asset.category} (${asset.percentage}%):`, 25, yPosition);
        yPosition += 8;
        ...
        if (fund) {
          pdf.text(`Primary: ${fund.name}`, 30, yPosition);
          yPosition += 5;
      ...
      checkPageBreak(40);
      pdf.setFontSize(14);
      pdf.text('5. GROWTH PROJECTION', 20, yPosition);
      yPosition += 8;
      ...
      const projectionLines = [
        ...
        `Wealth Multiplication: ${((finalProjection?.portfolioValue || 0) / (finalProjection?.totalInvested || 1)).toFixed(1)}x`
      ];
      ...
      checkPageBreak(30);
      pdf.setFontSize(14);
      pdf.text('6. TAX OPTIMIZATION STRATEGY', 20, yPosition);
      yPosition += 8;
      
      pdf.setFontSize(10);
      const taxLines = [
        '• Utilize ELSS funds for 80C deduction (up to ₹1.5L)',
        '• Long-term capital gains optimization after 1 year',
        '• Tax-harvesting opportunities for loss offset',
        '• NPS investment for additional 80CCD deduction',
      ...
      checkPageBreak(25);
      pdf.setFontSize(14);
      pdf.text('7. REBALANCING STRATEGY', 20, yPosition);
      yPosition += 8;
      ...
      checkPageBreak(25);
      pdf.setFontSize(14);
      pdf.text('8. BEHAVIORAL INSIGHTS & RECOMMENDATIONS', 20, yPosition);
      yPosition += 8;
      
      pdf.setFontSize(10);
      pdf.text(`Investor Type: ${behavioralInsights.investorType}`, 25, yPosition);
      yPosition += 6;
      pdf.text(`Discipline Score: ${behavioralInsights.disciplineScore}/100`, 25, yPosition);
      yPosition += 8;
      
      pdf.text('Key Recommendations:', 25, yPosition);
      yPosition += 6;
      behavioralInsights.recommendations.slice(0, 3).forEach(rec => {
      ...
      toast({
        ...
        description: "Your comprehensive investment strategy report has been downloaded successfully.",
      });
