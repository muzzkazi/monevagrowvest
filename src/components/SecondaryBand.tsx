...
const SecondaryBand = () => {
  ...
  return (
    <div className="bg-gradient-to-r from-blue-950/50 via-slate-900/35 to-blue-950/50 backdrop-blur-md text-white py-1.5 w-full z-20 overflow-hidden flex flex-col justify-center relative border-b border-blue-400/10">
      {/* First line - Indices */}
            ...
            return (
              <div key={`index-${index}`} className="flex items-center whitespace-nowrap flex-shrink-0">
                <div className="flex items-center gap-2 px-6">
                  ...
                  <span className={`flex items-center gap-1 text-sm ${isPositive ? 'text-green-300' : 'text-red-300'}`}>
            ...
            return (
              <div key={`stock-${index}`} className="flex items-center whitespace-nowrap flex-shrink-0">
                <div className="flex items-center gap-2 px-3">
                  ...
                  {isLoaded && (
                    <>
                      <span className={`text-xs ${isPositive ? 'text-green-300' : 'text-red-300'}`}>
                        {stock.change}
                      </span>
                      <span className={`text-xs ${isPositive ? 'text-green-300' : 'text-red-300'}`}>
                        ({stock.percent})
