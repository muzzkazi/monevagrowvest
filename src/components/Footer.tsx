...
const Footer = () => {
  ...
  return (
    <footer className="bg-gradient-primary text-white py-10 relative overflow-hidden">
      <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,')]"></div>
      <div ref={ref} className="container mx-auto px-4 relative z-10">
        <div className={`grid md:grid-cols-4 gap-6 mb-8 transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <div className={`space-y-3 transition-all duration-500 delay-100 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          ...
          <div className={`transition-all duration-500 delay-200 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            <h4 className="font-display font-semibold mb-4 text-lg">Premium Services</h4>
          ...
          <div className={`transition-all duration-500 delay-300 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            <h4 className="font-display font-semibold mb-4 text-lg">Smart Tools</h4>
            <ul className="space-y-2 text-white/80">
        ...
        <div className={`border-t border-white/20 pt-6 transition-all duration-700 delay-500 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
